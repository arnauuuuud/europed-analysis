...
for shot, dda in zip(list_shot, list_dda):
    ...
    for (eta, marker) in zip([0, 1], ['D', '^']):
        try:
            ...
            # Use the axes objects to plot
            if eta == 0:
                ax2.scatter(betan, alpha, c=color, marker=marker)
            if eta == 1:
                ax3.scatter(betan, alpha, c=color, marker=marker)
...
ax2.set_ylabel(r'$\alpha_{\mathrm{max}}$')
ax2.set_ylim(bottom=0)
ax2.set_xlim(left=0)
ax3.set_xlabel(global_functions.betan_label)
ax3.set_ylabel(r'$\alpha_{\mathrm{max}}$')
