...
def main(list_input, crit, crit_value, ypar, xpar, exclud_mode):
    ...
    for color,cv,em in zip(colors, crit_value, exclud_modes_good):
        print(color)
        if em is None or len(em) == 0:
            max_n = 50
        else:
            em = [int(a) for a in em]
            max_n = np.min(em)-10
        
        label = fr'{cv} - $n \leq {max_n}$'
        ym = add_contours_to_plot.main(ax, [color], list_input, crit, [cv], ypar, xpar, em, nscan=True, label=label)
        ymax = max(ymax, ym)
