...
def main():
    ...
    gs = gridspec.GridSpec(9, 4, width_ratios=[10, 1, 1, 1], hspace=0, wspace=0)
    ...
    ax_textbox_1 = plt.subplot(gs[0,1:3]) 
    eurobox_1 = TextBox(ax_textbox_1, '', initial='f4_eta')
    def update_euroname_1(val):
        euroname_1[0] = val.split(',')
    eurobox_1.on_submit(update_euroname_1)


    # Second text box: NAME EUROPED: modify euroname_var1 (optional)
    ax_textbox_2 = plt.subplot(gs[1,1:3]) 
    eurobox_2 = TextBox(ax_textbox_2, '', initial='0,1,2')
    def update_euroname_2(val):
        euroname_2[0] = val.split(',')
    eurobox_2.on_submit(update_euroname_2)


    # Third text box: NAME EUROPED: modify euroname_bas2 (optional)
    ax_textbox_3 = plt.subplot(gs[2,1:3]) 
    eurobox_3 = TextBox(ax_textbox_3, '', initial='_ds')
    def update_euroname_3(val):
        euroname_3[0] = val.split(',')
    eurobox_3.on_submit(update_euroname_3)


    # Fourth text box: NAME EUROPED: modify euroname_var2 (optional)
    ax_textbox_4 = plt.subplot(gs[3,1:3]) 
    eurobox_4 = TextBox(ax_textbox_4, '', initial='0')
    def update_euroname_4(val):
        euroname_4[0] = val.split(',')
    eurobox_4.on_submit(update_euroname_4)


    # Fifth box: Radiobuttons for x-axis parameter
    ax_x_parameter = plt.subplot(gs[:3,3])
    ...
    # Sixth box: Envelope
    ax_checkbox_envelope = plt.subplot(gs[4,1:3])
    ...
    ax_textbox_crit_value = plt.subplot(gs[5,1:3])
    ...
    # Eigth box: crit
    ax_radiobutton_crit = plt.subplot(gs[6:8,1:3])
    ...
    ax_checkbox_mode = plt.subplot(gs[3:8,3])
    ...
    def run_on_click(event):
        ...
        run(euroname_1[0], euroname_2[0], euroname_3[0], euroname_4[0], x_parameter[0])
...
def run(euroname_1, euroname_2, euroname_3, euroname_4, x_parameter):

    europed_names = [e1+e2+e3+e4 for e1 in euroname_1 for e2 in euroname_2 for e3 in euroname_3 for e4 in euroname_4]

    print(f'FULL LIST: {europed_names}')
    print(f'X_PARAMETER: {x_parameter}')
