...
from hoho import useful_recurring_functions, europed_analysis, global_functions, startup, europed_analysis_2
import matplotlib.pyplot as plt
    ...
    for iplot,europed_run in enumerate(europed_names):
        try:
            ...
            if not envelope:
                ...
                # dict_gamma = europed_analysis_2.get_gammas(europed_run, crit=crit)
                # x_parameter_list = europed_analysis_2.get_x_parameter(europed_run, x_parameter=x_parameter)
                # dict_gamma = europed_analysis_2.filter_dict(dict_gamma, list_consid_mode)
            ...
            if vline:
                has_unstable, x_crit, critn = europed_analysis_2.find_critical(x_param, deltas, dict_gamma, crit_value)
                if has_unstable:
                    if not envelope:
                        colorvline = 'r'
                    else:
