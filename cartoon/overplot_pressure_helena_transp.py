...
def main(europed_name, profile):
    ...
    for time_i in [5.51]:
        ...
        pressure_profile = interp_func(time_i)

        interp_func2 = interpolate.interp1d(time, x, kind='linear', axis=0)
        x_profile = interp_func2(time_i)
        ...
        pressure_w_fi = np.array(TRANSPDat.variables['BTTOT'])
        ...
        plt.plot(x_profile, pressure_profile,label ='BETA')
