...
if __name__ == '__main__':
    ...
    for xy in ['alphapeped','nepedteped','sepedwidthbis','betapbetan']:
        for (exclude_modes,marker) in zip([[],[50],[50,40],[50,40,30]],['s','^','o','P']):
            ...
            if exclude_modes is None or len(exclude_modes)==0:
                max_n = 50
            else:
                max_n = np.min(exclude_modes)-10
            ax.text(0.05,0.05, fr'$n \leq {max_n}$', transform=ax.transAxes)
            custom_entries = []
            ...
            if crit_value == 0.1:
                crit_value = '0.10'
            plt.savefig(f'/home/jwp9427/work/figures/2025-01-14_Gather_all/overall_fw_eta1_{xy}_nmax{max_n}.png')