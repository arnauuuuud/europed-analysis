...
def argument_parser():
    ...
    return args.original_name, args.extension_name, args.option, args.modes, args.deltas, args.is_fixed_width
...
def merge_single_profile(original_file, extension_file, modes, delta, is_fixed_width):
    ...
    for n in modes:
        ...
        try:
            ...
            try:
                ...
                original_file['scan'][profile_original].create_group('castor')
                original_file['scan'][profile_original]['castor'].create_group(str(n))
...
def merge(original_name, extension_name, option, modes, deltas_to_put, is_fixed_width):
    ...
    with h5py.File(f'{original_name}.h5', 'a') as original_file, h5py.File(f'{extension_name}.h5', 'r') as extension_file:
        ...
        for key in extension_file['scan'].keys():
            ...
            # if delta is already in the original file
                ...
                try:
                    if option == 'replace':
                        merge_single_profile(original_file, extension_file, modes, delta_extension, is_fixed_width)
            ...
            if letsadd:
                ...
                for key in new_group.keys():
                    ...
                    # try:
                    #     del original_file['scan'][name_new_profile]['castor']
                    # except KeyError:
                    #     print('No CASTOR results in the new file')

                    # original_file['scan'][name_new_profile].create_group('castor')
                    # new_group_castor = new_group['castor']
                    # for mode in new_group_castor.keys():
                    #     if int(mode) in modes:
                    #         new_group_castor.copy(mode, original_file['scan'][name_new_profile]['castor'])
...
def merge_files(original_name, extension_name, option, modes, deltas, is_fixed_width):
    copy_original_file(original_name, extension_name, option)
    h5_manipulation.decompress_gz(extension_name)
    merge(original_name, extension_name, option, modes, deltas, is_fixed_width)
...
if __name__ == '__main__':
    original_name, extension_name, option, modes, deltas, is_fixed_width = argument_parser()
    merge_files(original_name, extension_name, option, modes, deltas, is_fixed_width)