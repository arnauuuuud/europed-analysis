...
def main(inputfile, **kwargs):
    ...
    if 'run_name' in kwargs:
        if kwargs['run_name'] != inputfile:
            ...
            os.system(f"cp {os.environ['EUROPED_DIR']}scan_inputs/{inputfile} {os.environ['EUROPED_DIR']}scan_inputs/{kwargs['run_name']}")
            inputfile = kwargs['run_name']
    
    # Setting parameters in inputfile
    written = europed.modify_inputfile(f"{os.environ['EUROPED_DIR']}scan_inputs/{inputfile}", **kwargs)
