...
def main(ax, color, list_input, crit_value, ypar, xpar, exclud_mode):
    cm = 1/2.54

    if list_input == 'tanrs':
        europed_namess = [[f'tan_eta{eta}_rs{rs}_neped2.57_betap1.3' for rs in ['0.0','0.01','0.02','0.022','0.03','0.04']] for eta in [1]]
        if xpar is None:
            xpar = 'frac'

    elif list_input == 'tanneped':
        europed_namess = [[f'tan_eta{eta}_rs0.022_neped{neped}_betap1.3' for neped in ['1.57','2.07','2.57','3.07','3.57','4.07']] for eta in [1]]
        if xpar is None:
            xpar = 'neped'

    elif list_input == 'tanbeta':
        europed_namess = [[f'tan_eta{eta}_rs0.022_neped2.57_betap{betap}' for betap in ['0.55','0.7','0.85','1.0','1.15','1.3','1.45']] for eta in [1]]
        if xpar is None:
            xpar = 'betap'

    elif list_input == 'tbrs':
        europed_namess = [[f'tb_eta{eta}_rs{rs}_neped3_betap0.85' for rs in ['0.01','0.02','0.03','0.04','0.08']] for eta in [1]]
        if xpar is None:
            xpar = 'frac'

    elif list_input == 'tbneped':
        europed_namess = [[f'tb_eta{eta}_rs0.04_neped{neped}_betap0.85' for neped in [1.5,2,2.5,3,3.5,4]] for eta in [1]]
        if xpar is None:
            xpar = 'neped'

    elif list_input == 'tbbeta':
        europed_namess = [[f'tb_eta{eta}_rs0.04_neped3_betap{betap}' for betap in ['0.55','0.7','1.0','1.15','1.3']] for eta in [1]]
        if xpar is None:
            xpar = 'betap'
    ...
    for i,europed_names in enumerate(europed_namess):
        ...
        try:
            ymax.append(np.nanmax([yi for (yi,zi) in zip(y,z) if zi < 1.1*crit_value]))
        except ValueError:
            ymax.append(0)
