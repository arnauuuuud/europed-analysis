...
def give_matching_x_with_deltas(delta_input, x_input, delta_to_plot):
    ...
    mask = [d in delta_to_plot for d in delta_input]
    x_input = np.array(x_input)
...
def get_gammas(filename, crit="alfven", fixed_width=False):
    ...
    with h5py.File(filename + '.h5', 'r') as hdf5_file:
        ...
        while stability_code == '':
            if 'castor' in list(hdf5_file['scan'][list_profile[i]].keys()):
                stability_code = 'castor'
            elif 'mishka' in list(hdf5_file['scan'][list_profile[i]].keys()):
        ...
        for i in list_profile:
            try:
                list_mode += list(hdf5_file['scan'][i][stability_code].keys())
            except KeyError:
                pass
