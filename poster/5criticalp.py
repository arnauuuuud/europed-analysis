...
def run(europed_names, y_parameter, crit, crit_value, list_consid_mode, shown, whichxparameter, xlabel, is_frac):
    ...
    for y_list,n_list,label,y_list_max,y_list_min in zip(y_lists, n_lists,labels,y_lists_max,y_lists_min):
        try:
            ...
            ax.plot(x_list_plot, y_list, marker='p', label=label, color=color, mec='black', mew=1, markersize=10)
...
run(europed_names, y_parameter, crit, crit_value, list_consid_mode, shown, whichxparameter, xlabel, is_frac)


ax.axvspan(1.136,2.143,alpha=0.2,color='forestgreen')
mean = (1.136+2.143)/2
ax.text(mean,0.5,r'$\eta_{\mathrm{neo}}$',ha='center',va='center',fontsize=40, color='forestgreen')

plt.savefig('/home/jwp9427/cococo/5')
plt.close()
