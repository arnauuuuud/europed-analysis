...
def main(prefixes, variations, suffix, crit, crit_value, ypar, exclud_mode, consid_mode_input,plot_frac):
    ...
    tab_nesepneped = [[0.494215, 0.0134352, 3.71118, 0.289132, blue],
        [0.364361, 0.0115456, 3.98674,     0.352758, blue],
        [0.471215, 0.0205522, 3.20297,     0.373003, green],
        [0.417765, 0.0130768, 3.57862,     0.390023, green],
        [0.508334, 0.0116359, 3.03715,     0.322383, pink],
        [0.653416, 0.0171798, 2.96227,     0.364966, pink],
        [0.619142, 0.0197673, 2.84640,     0.271229, pink]]


    tab_nesepneped = np.array(tab_nesepneped)


    nesepneped = tab_nesepneped[:,0]
    nesepneped = [float(i) for i in nesepneped]
    nesepneped_error = tab_nesepneped[:,1]
    nesepneped_error = [float(i) for i in nesepneped_error]
    peped = tab_nesepneped[:,2]
    peped = [float(i) for i in peped]
    peped_error = tab_nesepneped[:,3]
    peped_error = [float(i) for i in peped_error]
    colors = tab_nesepneped[:,4]
    markers = ['o','o','s','s','*','*','*']

    for x,xerr,y,yerr,color,marker in zip(nesepneped,nesepneped_error,peped,peped_error,colors,markers):
        size=200
        if marker == '*':
            size=400
    ...
    ax3.set_xlim(left=0.3,right=0.7)
    ax3.set_ylim(bottom=0,top=5.9)
    ...
    plt.savefig('/home/jwp9427/cococo/8')
    plt.close()
